...
# ```
# 
# 해당사이트에 접속한 후, 명언에 해당하는 부분에 마우스 커서를 올려둔 후 마우스 오른쪽 버튼을 클릭하고 [검사]를 선택하면 개발자도구 화면이 나타난다. 여기서 해당 글자가 HTML 내에서 어떤 부분에 위치하는지 확인할 수 있다. 각 네모에 해당하는 부분은 [class가 quote인 div 태그]에 위치하고 있으며, 명언은 그 하부의 [class가 text인 span 태그]에, 말한 사람은 [span 태그 하단의 class가 author인 small 태그]에 위치하고 있다. 마지막으로 말한 사람에 대한 정보인 (about)의 링크는 [a 태그 href 속성]의 속성값에 위치하고 있다.
# 
...
# `find_all()` 함수를 이용할 경우 원하는 태그의 값들을 찾아올 수 있다. 먼저 태그에 해당하는 'div'를 입력하고, class 이름인 'quote'를 입력한다. class라는 키워드는 파이썬에서 클래스를 만들 때 사용하는 키워드이므로 언더바(\_)를 통해 중복을 피해준다. 조건에 만족하는 결과가 리스트 형태로 반환되므로, 첫번째 내용만 확인해보면 `div class="quote"`에 해당하는 내용을 찾아왔으며, 이제 여기서 [class가 text인 span 태그]에 해당하는 내용을 추가로 찾도록 하자.
...
# 4. 명언과 말한이, 링크에 해당하는 내용을 각각 추출한다.
...
# ```
# 
# 이 중 뉴스의 제목에 해당하는 텍스트만 추출하도록 하고자 한다. 개발자도구 화면을 통헤 제목에 해당하는 부분은 [dl 태그 → class가 articleSubject 인 dd 태그 → a 태그 중 title 속성]에 위치하고 있음을 확인할 수 있다.
# 
